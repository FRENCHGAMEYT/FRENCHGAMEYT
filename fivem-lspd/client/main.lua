...
function ToggleDuty()
    if not IsPolice() then
        lib.notify({ title = 'LSPD', description = 'Vous n\'êtes pas policier.', type = 'error' })
        return
    end
    isOnDuty = not isOnDuty
    TriggerServerEvent('lspd:setDuty', isOnDuty)
    if isOnDuty then
        lib.notify({ title = 'LSPD', description = 'Vous êtes maintenant en service.', type = 'success', duration = 4000 })
        TriggerEvent('lspd:onDuty')
    else
        lib.notify({ title = 'LSPD', description = 'Vous avez terminé votre service.', type = 'inform', duration = 4000 })
...
RegisterCommand('lspdduty', function() ToggleDuty() end, false)
